...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.item_types = {
            'VanillaScoop': 'scoop', 'ChocolateScoop': 'scoop',
            'CaramelTopping': 'topping', 'SprinklesTopping': 'topping'
        }
        self.item_colors = {
            'VanillaScoop': '#FFFACD', # LemonChiffon
            'ChocolateScoop': '#8B4513', # SaddleBrown
            'CaramelTopping': '#D2691E', # Chocolate
            'SprinklesTopping': '#FF69B4'  # HotPink (sebagai representasi)
        }
    ...
    def setup_ui(self):
        ...
        products_scroll_frame = ctk.CTkScrollableFrame(left_frame, height=500, label_text="Pilih Item", label_font=("Arial", 20, "bold"))
        products_scroll_frame.grid(row=0, column=0, padx=0, pady=0, sticky="nsew")
        ...
        # --- Konfigurasi Grid di action_frame ---
        # Konfigurasi agar kedua kolom (0 dan 1) memiliki bobot (weight) yang sama (1),
        # sehingga kedua tombol akan membagi ruang secara merata dan melebar (flex).
        ...
        # Gunakan grid, bukan pack. Tempatkan di baris 0, kolom 0. Gunakan 'sticky="ew"' agar tombol mengisi kolom.
        ...
        # Gunakan grid, bukan pack. Tempatkan di baris 0, kolom 1. Gunakan 'sticky="ew"' agar tombol mengisi kolom.
        self.cancel_button.grid(row=0, column=1, padx=5, pady=5, sticky="ew")

        # Masalah 3: Hiasan slot uang
        money_slot = ctk.CTkFrame(action_frame, height=10, fg_color="black", border_width=2, border_color="grey")
        money_slot.grid(row=1, column=0, columnspan=2, padx=40, pady=10, sticky="ew")
    ...
    def handle_input(self, input_symbol):
        ...
        if self.vm_dfa.current_state == 'DispensingItem':
            ...
            if self.vm_dfa.current_state == 'TransactionComplete':
                self.show_change()
    ...
    def update_gui(self, message):
        ...
        for name, button in self.product_buttons.items():
            is_active = state in ['Idle', 'WaitingForSelection'] and self.vm_dfa.stock.get(name, 0) > 0
        ...
        self.cancel_button.configure(state="normal" if state not in ['Idle', 'TransactionComplete', 'DispensingItem'] else "disabled")
        self.take_change_button.configure(state="normal" if state == 'TransactionComplete' else "disabled")

    def play_sound(self):
        if self.notification_sound:
            Thread(target=lambda: self.notification_sound.play(), daemon=True).start()
    ...
    def show_change(self):
        ...
        # Masalah 4: Logika untuk "flex-wrap"
        max_items_per_row = 3 # Tentukan berapa banyak gambar uang per baris
